...
| 3. Somebody else can run it | The container build is verified rather than asserted, there is a deployment path past one compose file, and the enterprise surface (retention, audit export, SIEM, SSO/RBAC, multi-tenancy) exists for the buyers who require it | The container build is verified rather than asserted (CI builds the image and serves from it), and the enterprise surface — SSO/SAML, RBAC, multi-tenancy, signed audit export — is not implemented | Weeks to months, and only worth starting once gate 4 says which of them a real buyer asks for |
