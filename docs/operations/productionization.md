...
- **做完的**：[incidents.md](incidents.md)（怎么知道出事、第一分钟三条命令、按症状止血、事故中不许做的四件事、单人分级、复盘模板与复盘记录）、[key-rotation.md](key-rotation.md)（四类凭据在哪/泄露后果/先加后删的轮换步骤/轮换后必须跑演练）、[README.md](README.md)（按症状找手册 + 当前实际运行节奏 + 文档维护规则）。`START-HERE.md` 顶部加了指向索引的一行。
