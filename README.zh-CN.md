...
先 `node bin/agentgate.mjs serve`，打开它打印出来的地址上的 `/inventory.html`。把工具名清单粘进去，或选一个文本/JSON 文件，处理需要确认的匹配，填上你实际在用的版本，然后下载一份独立的 HTML 报告。对照发生在浏览器内存里、对着内嵌的索引快照：**清单不上传、不保存，不扫描你的机器，也不执行任何工具。**
