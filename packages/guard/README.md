...
| `install-hooks` | AG-INSTALL-001 | `preinstall`/`install`/`postinstall`/`prepare` scripts that fetch-and-exec, decode base64, or run inline code that can fetch, spawn or rewrite files. The inline program is read rather than inferred from its shape: a print-only install banner is not a finding, and a fixed command such as `execSync('npm run build')` is a medium, not a critical |
...
| `supply-chain` | AG-SUPPLY-001 … 003 | dependencies resolved from a mutable source — a remote ref (git/http) at high, a path inside the repository (`file:`/`link:`, which no consumer can resolve) at medium — versions floating on `*` or `latest`, dependencies with no committed lockfile. A dev or peer dependency is reported at a lower severity than a runtime one, because it never reaches a consumer of the package |
