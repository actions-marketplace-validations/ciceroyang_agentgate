...
import { contentProvenance, isExactNpmVersion, isNpmPackageName } from "../provenance.mjs"
...
/** Check exactly the manifest fields passed to agent-guard and bind their content. */
export function scanManifest(row, manifest, manifestFindings) {
  const base = { server: row.server, package: row.package, version: row.version }
  const identity = { registry: row.registryType ?? "npm", name: row.package, version: row.version }
  const scope = "packageManifest/v1: exact npm version manifest JSON fields scripts, dependencies, devDependencies, optionalDependencies; excludes referenced script bodies and package artifact contents"
  const exact = identity.registry === "npm" && isNpmPackageName(row.package) && isExactNpmVersion(row.version) && manifest &&
    typeof manifest === "object" && !Array.isArray(manifest) && manifest.name === row.package && manifest.version === row.version
  if (!exact) {
    return {
      ...base, status: "metadata-unavailable", findings: [],
      provenance: contentProvenance({ package: identity, scope, input: { status: "missing-exact-manifest" }, complete: false }),
    }
  }
  ...
  // Parse the exact JSON supplied to the checker so omitted fields and JSON values
  // match its input, while canonical hashing ignores object insertion order.
  const input = { status: "present", manifestFields: JSON.parse(text) }
  const provenance = contentProvenance({ package: identity, scope, input, complete: false })
  try {
    const findings = manifestFindings(text, "package.json")
    if (!Array.isArray(findings)) throw new TypeError("manifest checker did not return findings")
    return { ...base, status: findings.length > 0 ? "findings" : "clean", findings, provenance: { ...provenance, complete: true } }
  } catch (error) {
    return { ...base, status: "check-failed", error: String(error && error.message || error), findings: [], provenance }
  }
}
...
async function main() {
  ...
  const agentGuardRoot = resolve(args.agentGuard || process.env.AGENT_GUARD || resolve(HERE, "..", "..", "guard"))
  ...
  async function fetchManifest(name, version) {
    if (!isNpmPackageName(name) || !isExactNpmVersion(version)) return null
  ...
  async function worker() {
    while (cursor < rows.length) {
      ...
      results.push(scanManifest(row, manifest, manifestFindings))
    }
  }
...
if (isMain) main().catch((error) => { console.error("guard-scan: " + error.message); process.exitCode = 1 })
